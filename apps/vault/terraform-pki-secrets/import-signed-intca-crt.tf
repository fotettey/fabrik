...
resource "vault_pki_secret_backend_intermediate_set_signed" "ogd_pki_int_ca_v1_signed_cert" {
 depends_on   = [vault_mount.ogd_pki_int_ca_v1]
 backend      = vault_mount.ogd_pki_int_ca_v1.path

 ## Reference the signed and received INT_CA CRT from BundesDruckerei:
